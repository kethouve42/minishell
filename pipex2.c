...
/*   Updated: 2024/06/07 16:08:13 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	start2(t_pipe *pipee, t_ms *ms_data)
{
	...
	if (execution2(ms_data, pipee->j, pipee) == 1)
		exit(0);
	execve(ft_strjoin(pipee->env[pipee->ij[pipee->j]],
			pipee->cmds[pipee->j][0]), pipee->cmds[pipee->j], ms_data->envp);
...
void	seearsh2(t_pipe *pipee, t_ms *ms_data)
{
	int		i;
...
int	exec2(char ***cmd, t_ms *ms_data)
{
	...
	seearsh2(pipee, ms_data);
	exec_exec(pipee, ms_data);
